...
<div class="container-fluid px-0">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-1">Data Barang</h2>
            <p class="text-muted mb-0">Kelola seluruh data inventaris laboratorium</p>
        </div>
        <div>
        ...
        <div class="col-md-3 col-sm-6">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <p class="text-muted mb-1 small">KONDISI BAIK</p>
        ...
        <div class="card-body p-3">
            <form action="{{ route('barang.index') }}" method="GET" class="row g-3">
                ...
                <div class="col-md-2 col-sm-6">
                    ...
                    <select name="merk" class="form-select form-select-sm rounded-pill">
                ...
                <div class="col-md-2 col-sm-6">
                    ...
                    <select name="kondisi" class="form-select form-select-sm rounded-pill">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-header bg-transparent border-0 pt-3">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <h5 class="fw-semibold mb-0">
                    <i class="fas fa-table-list me-2 text-primary"></i>Daftar Inventaris Barang
                </h5>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-bordered table-hover mb-0 align-middle" style="min-width: 1200px; font-size: 0.85rem;">
                    <thead class="table-light">
                        <tr>
                            <th class="ps-2">Merk Alat</th>
                            <th>Nama Alat</th>
                            <th>Deskripsi</th>
                            <th class="text-success">Baik</th>
                            <th class="text-warning">Rusak</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-2">{{ $item->merk ?? '-' }}</td>
                            <td class="fw-semibold">{{ $item->nama_barang }}</td>
                            <td>{{ Str::limit($item->deskripsi, 50) }}</td>
                            <td>{{ $item->jumlah_baik ?? 0 }}</td>
                            ...
                            <td>
                                @php
                                    ...
                                    $kapasitasTampil = $kapasitasAngka ?: '-';
                                @endphp
                                {{ $kapasitasTampil }}
                            </td>
                            <td>{{ $item->keterangan ?? '-' }}</td>
                            <td>
                                @if($item->penanggungJawab->count())
                                    {{ $item->penanggungJawab->pluck('nama_pj')->implode(', ') }}
                                @else - @endif
                            </td>
                            <td class="pe-2">
                                <a href="{{ route('barang.edit', $item->id_barang) }}" class="btn btn-sm btn-outline-primary rounded-pill me-1" style="padding: 0.2rem 0.5rem; font-size: 0.75rem;" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('barang.destroy', $item->id_barang) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger rounded-pill" style="padding: 0.2rem 0.5rem; font-size: 0.75rem;" onclick="return confirm('Yakin hapus barang ini?')" title="Hapus">
        ...
        <div class="card-footer bg-transparent border-0 py-3">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div class="small text-muted">
                    Menampilkan {{ $barang->firstItem() ?? 0 }} sampai {{ $barang->lastItem() ?? 0 }} dari {{ $barang->total() }} barang
                </div>
                <div>
